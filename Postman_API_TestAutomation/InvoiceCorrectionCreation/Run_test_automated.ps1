...
# === SEKCJA SCIEZEK ===
# Folder projektu
$projectFolder = Join-Path -Path $basePath -ChildPath "Postman_API_TestAutomation\InvoiceCorrectionCreation"
$logFolder = Join-Path -Path $projectFolder -ChildPath "user_logs"
# Sciezki do nowych plików
$collectionPath = Join-Path -Path $projectFolder -ChildPath "AAT_InvoiceCorrection.postman_collection.json"
$environmentPath = Join-Path -Path $projectFolder -ChildPath "InvoiceCorrection_Creation.postman_environment.json"
$dataPath = Join-Path -Path $projectFolder -ChildPath "TD_AAT_IvocieCorrectionCreation.json"
# === KONIEC SEKCJI ===
...
try {
    ...
    # --- Filtrowanie uzytkownikow na podstawie zakresu ---
    if ($endUser -gt 0) {
