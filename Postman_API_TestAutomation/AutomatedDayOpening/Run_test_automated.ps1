...
# === SEKCJA SCIEZEK ===
# Folder projektu
$projectFolder = Join-Path -Path $basePath -ChildPath "Postman_API_TestAutomation\AutomatedDayOpening"
$logFolder = Join-Path -Path $projectFolder -ChildPath "user_logs"
# Sciezki do nowych plików
$collectionPath = Join-Path -Path $projectFolder -ChildPath "AAT_Session_StartOfDay.postman_collection.json"
$environmentPath = Join-Path -Path $projectFolder -ChildPath "Session_StartOfDay.postman_environment.json"
$dataPath = Join-Path -Path $projectFolder -ChildPath "TD_AAT_POS_DATA.json"
# === KONIEC SEKCJI ===
