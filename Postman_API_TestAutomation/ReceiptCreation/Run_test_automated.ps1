...
# === SEKCJA SCIEZEK ===
# Folder projektu
$projectFolder = Join-Path -Path $basePath -ChildPath "Postman_API_TestAutomation\ReceiptCreation"
$logFolder = Join-Path -Path $projectFolder -ChildPath "user_logs"
# Sciezki do nowych plików
$collectionPath = Join-Path -Path $projectFolder -ChildPath "AAT_ReceiptCreation.postman_collection.json"
$environmentPath = Join-Path -Path $projectFolder -ChildPath "ReceiptCreation.postman_environment.json"
$dataPath = Join-Path -Path $projectFolder -ChildPath "TD_AAT_ReceiptCreation_Parallel.json"
# === KONIEC SEKCJI ===
