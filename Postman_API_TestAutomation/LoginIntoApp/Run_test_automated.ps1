...
# === SEKCJA SCIEZEK ===
# Folder projektu
$projectFolder = Join-Path -Path $basePath -ChildPath "Postman_API_TestAutomation\LoginIntoApp"
$logFolder = Join-Path -Path $projectFolder -ChildPath "user_logs"
# Sciezki do nowych plików
$collectionPath = Join-Path -Path $projectFolder -ChildPath "AAT_Logging_Fixed.postman_collection.json"
$environmentPath = Join-Path -Path $projectFolder -ChildPath "Logging_Sessions_Fixed.postman_environment.json"
